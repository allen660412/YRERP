...
using YR.ERP.DAL.YRModel.Reports.Inv.Invr521;
using System.Linq;

namespace YR.ERP.Forms.Inv
{
    public partial class FrmInvr521 : YR.ERP.Base.Forms.FrmReportBase
    {
        #region Property
        BasBLL BoBas = null;
        #endregion
        ...
        protected override Boolean WfSetVar()
        {
            ...
            this.strFormID = "XXX";
             */
            this.StrFormID = "invr521";
            TabMaster.ReportName = @"\Inv\invr521.mrt";
        ...
        protected override bool WfExecReport(Stimulsoft.Report.StiReport pReport)
        {
            ...
            try
            {
                invr521Model = DrMaster.ToItem<vw_invr521>();
                resultList = new List<Master>();
                queryInfoList = new List<QueryInfo>();
                #region range 處理
                if (!GlobalFn.varIsNull(invr521Model.ipa01))
                {
                ...
                if (!GlobalFn.varIsNull(invr521Model.ipa03_s))
                {
                    sbQuerySingle.AppendLine("AND ipa03>=@ipa03_s");
                    sqlParmList.Add(new SqlParameter("@ila02_s", invr521Model.ipa03_s));
                }
                if (!GlobalFn.varIsNull(invr521Model.ipa03_e))
                {
                    sbQuerySingle.AppendLine("AND ipa03<=@ipa03_e");
                    sqlParmList.Add(new SqlParameter("@ipa03_e", invr521Model.ipa03_e));
                }
                #endregion
                ...
                foreach (Master masterModel in resultList)
                {
                    ...
                    //是否列印數量
                    if (invr521Model.print_qty_yn == "Y")
                    {
                    ...
                    {
                        masterModel.qty1_str = "";
                        masterModel.qty2_str = "";
                        masterModel.dif_qty_str = "";
                    }
                }

                if (invr521Model.dif_yn == "Y")//只顯示差異
                {
