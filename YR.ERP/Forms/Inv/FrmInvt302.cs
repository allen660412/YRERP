...
{
    public partial class FrmInvt302 : YR.ERP.Base.Forms.FrmEntryMDBase
    {
        #region Property
        BasBLL BoBas = null;
        InvBLL BoInv = null;
        ...
        protected override Boolean WfIniTabDetailInfo()
        {
            ...
            this.TabDetailList[0].ViewTable = "vw_invt302s";
        ...
        {
            vw_invt302 masterModel = null;
            try
            {
                if (DrMaster != null && DrMaster.RowState != DataRowState.Detached)
                {
                    masterModel = DrMaster.ToItem<vw_invt302>();
        ...
        protected override bool WfPickClickOnEditMode(object sender, string pColName, DataRow pDr)
        {
            try
            {
                ...
                #region 單頭-pick vw_invt302
                if (pDr.Table.Prefix.ToLower() == "vw_invt302")
                {
                ...
                #region 單身-pick vw_invt302s
                if (pDr.Table.Prefix.ToLower() == "vw_invt302s")
                {
                    masterModel = DrMaster.ToItem<vw_invt302>();
                    detailModel = pDr.ToItem<vw_invt302s>();
                    switch (pColName.ToLower())
                    {
                        case "igb03"://料號
        ...
        protected override bool WfItemCheck(object sender, ItemCheckInfo e)
        {
            ...
            vw_invt302 masterModel = null;
            vw_invt302s detailModel = null;
            List<vw_invt302s> detailList = null;
            bab_tb l_bab = null;
            ...
            try
            {
                ...
                #region 單頭-pick vw_invt302
                if (e.Row.Table.Prefix.ToLower() == "vw_invt302")
                {
                ...
                #region 單身-pick vw_invt302s
                if (e.Row.Table.Prefix.ToLower() == "vw_invt302s")
                {
                    uGrid = sender as UltraGrid;
                    detailModel = e.Row.ToItem<vw_invt302s>();
                    detailList = e.Row.Table.ToList<vw_invt302s>();
        ...
        protected override bool WfFormCheck()
        {
            ...
            try
            {
                ...
                foreach (DataRow drTemp in TabDetailList[iChkDetailTab].DtSource.Rows)
                {
                    ...
                    detailModel = drTemp.ToItem<vw_invt302s>();
                    chkColName = "igb02";   //項次
        ...
        private void WfConfirm()
        {
            ...
            try
            {
                ...
                foreach (DataRow dr in TabDetailList[0].DtSource.Rows)
                {
                    ...
                    //更新庫存交易歷史檔
                    if (BoInv.OfStockPost("invt302", igaModel, igbModel, LoginInfo, out errMsg) == false)
                    {
                        WfShowErrorMsg(errMsg);
                        DrMaster.RejectChanges();
                        WfRollback();
        ...
        private bool WfCancelConfirmChk()
        {
            ...
            try
            {
                ...
                detailList = TabDetailList[0].DtSource.ToList<vw_invt302s>();
                //檢查銷貨單的數量是否足夠做出庫
                detailList = TabDetailList[0].DtSource.ToList<vw_invt302s>();
