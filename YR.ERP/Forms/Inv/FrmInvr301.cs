...
using YR.ERP.DAL.YRModel.Reports.Inv.Invr301;

namespace YR.ERP.Forms.Inv
{
    public partial class FrmInvr301 : YR.ERP.Base.Forms.FrmReportBase
    {
        #region property
        ...
        public FrmInvr301(YR.ERP.Shared.UserInfo pUserInfo, vw_invr301 pInvr301Model, bool pAutoExecuted, bool pCloseAfterExecute)
        {
            InitializeComponent();

            this.LoginInfo = pUserInfo;
            this.Vw_Invr301 = pInvr301Model;
        ...
        protected override Boolean WfSetVar()
        {
            ...
            this.strFormID = "XXX";
             */
            this.StrFormID = "invr301";
            TabMaster.ReportName = @"\Inv\invr301.mrt";
        ...
        {
            vw_invr301 invr301Model = null;
            string msg;
            try
            {

                invr301Model = DrMaster.ToItem<vw_invr301>();
                if (GlobalFn.varIsNull(invr301Model.iga01)
                    && GlobalFn.varIsNull(invr301Model.iga02_s)
                    && GlobalFn.varIsNull(invr301Model.iga02_e)
                    )
                {
                    msg = "請至少輸入一個條件!";
                    WfShowErrorMsg(msg);
        ...
        protected override bool WfExecReport(Stimulsoft.Report.StiReport pReport)
        {
            ...
            try
            {
                ...
                if (!GlobalFn.varIsNull(invr301Model.iga01))
                {
                    ...
                    queryModel.Value = invr301Model.iga01;
                ...
                if (!GlobalFn.varIsNull(invr301Model.iga02_s))
                {
                    sbQuerySingle.AppendLine("AND iga02>=@iga02_s");
                    sqlParmList.Add(new SqlParameter("@iga02_s", invr301Model.iga02_s));
                }
                if (!GlobalFn.varIsNull(invr301Model.iga02_e))
                {
                    sbQuerySingle.AppendLine("AND iga02<=@iga02_e");
                    sqlParmList.Add(new SqlParameter("@iga02_e", invr301Model.iga02_e));
                }
                #endregion
                ...
                if (invr301Model.jump_yn.ToUpper() == "Y")
                {
