...
using YR.ERP.DAL.YRModel.Reports.Inv.Invr336;

namespace YR.ERP.Forms.Inv
{
    public partial class FrmInvr336 : YR.ERP.Base.Forms.FrmReportBase
    {
        #region Property
        BasBLL BoBas = null;
        #endregion
        ...
        protected override Boolean WfSetVar()
        {
            ...
            this.strFormID = "XXX";
             */
            this.StrFormID = "invr336";
            TabMaster.ReportName = @"\Inv\invr336.mrt";
        ...
        protected override bool WfItemCheck(object sender, ItemCheckInfo e)
        {
            ...
            try
            {
                ...
                {
                    case "ifa02_s":
                        if (!GlobalFn.varIsNull(e.Value) && !GlobalFn.varIsNull(invr336Model.ifa02_e))
                        {
                            if (invr336Model.ifa02_s > invr336Model.ifa02_e)
                            {
        ...
        {
            vw_invr336 invr336Model = null;
            string msg;
            try
            {
                invr336Model = DrMaster.ToItem<vw_invr336>();
                if (
                     GlobalFn.varIsNull(invr336Model.ifa02_s)
                    && GlobalFn.varIsNull(invr336Model.ifa02_e)
                    && GlobalFn.varIsNull(invr336Model.ifa01)
                    && GlobalFn.varIsNull(invr336Model.ifa03)
                    && GlobalFn.varIsNull(invr336Model.ifa04)
                    )
                {
                    msg = "請至少輸入一個條件!";
                    WfShowErrorMsg(msg);
            ...
            try
            {
                ...
                switch (pColName.ToLower())
                {
                    case "ifa01"://調撥單號
                        ...
                        messageModel.StrWhereAppend = " AND ifa00='1' ";
