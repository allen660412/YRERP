...
{
    public partial class FrmInvt331 : YR.ERP.Base.Forms.FrmEntryMDBase
    {
        #region Property
        BasBLL BoBas = null;
        InvBLL BoInv = null;
        ...
        protected override Boolean WfIniTabDetailInfo()
        {
            ...
            this.TabDetailList[0].ViewTable = "vw_invt331s";
        ...
        {
            vw_invt331 masterModel = null;
            try
            {
                if (DrMaster != null && DrMaster.RowState != DataRowState.Detached)
                {
                    masterModel = DrMaster.ToItem<vw_invt331>();
                ...
                else
                {
                    ...
                    WfSetControlReadonly(new List<Control> { ute_ifa06_c, ute_ifa08_c }, true);
                    WfSetControlReadonly(new List<Control> { ute_ifa08, ute_ifa09, udp_ifa10 }, true);
        ...
        protected override bool WfPickClickOnEditMode(object sender, string pColName, DataRow pDr)
        {
            ...
            {
                vw_invt331 masterModel = null;
                vw_invt331s detailModel = null;
                //this.MsgInfoReturned = new MessageInfo();
                MessageInfo messageModel = new MessageInfo();
                #region 單頭-pick vw_invt331
                if (pDr.Table.Prefix.ToLower() == "vw_invt331")
                {
                ...
                #region 單身-pick vw_invt331s
                if (pDr.Table.Prefix.ToLower() == "vw_invt331s")
                {
                    masterModel = DrMaster.ToItem<vw_invt331>();
                    detailModel = pDr.ToItem<vw_invt331s>();
                    switch (pColName.ToLower())
                    {
                        case "ifb03"://料號
        ...
        protected override bool WfItemCheck(object sender, ItemCheckInfo e)
        {
            ...
            vw_invt331 masterModel = null;
            vw_invt331s detailModel = null;
            List<vw_invt331s> detailList = null;
            bab_tb l_bab = null;
            ...
            try
            {
                ...
                #region 單頭-pick vw_invt331
                if (e.Row.Table.Prefix.ToLower() == "vw_invt331")
                {
                    switch (e.Column.ToLower())
                    {
                        case "ifa01":
                            if (GlobalFn.isNullRet(e.Value, "") == "")
                                return true;
                            if (BoBas.OfChkBabPKValid(GlobalFn.isNullRet(e.Value, ""), "inv", "21") == false)
                            {
                        ...
                        case "ifa06"://在途倉
                            ...
                            {
                                WfShowErrorMsg("無此可用在途倉,請確認!");
                                return false;
                            }
                            e.Row["ifa06_c"] = BoInv.OfGetIcb02(GlobalFn.isNullRet(e.Value, ""));
                            break;
                ...
                #region 單身-pick vw_invt331s
                if (e.Row.Table.Prefix.ToLower() == "vw_invt331s")
                {
                    uGrid = sender as UltraGrid;
                    detailModel = e.Row.ToItem<vw_invt331s>();
                    detailList = e.Row.Table.ToList<vw_invt331s>();
        ...
        protected override bool WfFormCheck()
        {
            ...
            try
            {
                ...
                chkColName = "ifa06";       //在途倉
                chkControl = ute_ifa06;
                if (GlobalFn.varIsNull(masterModel.ifa06))
                {
                    this.uTab_Master.SelectedTab = uTab_Master.Tabs[0];
                    chkControl.Focus();
                    msg = TabMaster.AzaTbList.Where(p => p.aza03 == chkColName).Select(p => p.aza04).FirstOrDefault();
                ...
                foreach (DataRow drTemp in TabDetailList[iChkDetailTab].DtSource.Rows)
                {
                    ...
                    detailModel = drTemp.ToItem<vw_invt331s>();
                    chkColName = "ifb02";   //項次
        ...
        protected override bool WfAppendUpdate()
        {
            ...
            try
            {
                ...
                foreach (DataRow dr in TabDetailList[0].DtSource.Rows)
                {
                    ...
                    if (BoInv.OfChkIccPKExists(ifbModel.ifb03, ifaModel.ifa06) == false)
                    {
                        if (dtIcc.Rows.Count > 0)
                        {
                            var drIccs = dtIcc.Select(string.Format("icc01='{0}' AND icc02='{1}'", ifbModel.ifb03, ifaModel.ifa06));
                            if (drIccs != null && drIccs.Length > 0)
                                continue;
                        }
                        drIcc = dtIcc.NewRow();
                        drIcc["icc01"] = ifbModel.ifb03;  //料號
                        drIcc["icc02"] = ifaModel.ifa06;
                        drIcc["icc03"] = "";
        ...
        private void WfConfirm()
        {
            ...
            try
            {
                ...
                foreach (DataRow dr in TabDetailList[0].DtSource.Rows)
                {
                    ...
                    //更新庫存交易歷史檔
                    if (BoInv.OfStockPost("invt331", ifaModel, ifbModel, LoginInfo, out errMsg) == false)
                    {
                        WfShowErrorMsg(errMsg);
                        DrMaster.RejectChanges();
                        WfRollback();
        ...
        private bool WfConfirmChk()
        {
            ...
            try
            {
                ...
                detailList = TabDetailList[0].DtSource.ToList<vw_invt331s>();
                //檢查撥出倉別的數量是否足夠做出庫
                detailList = TabDetailList[0].DtSource.ToList<vw_invt331s>();
        ...
        private bool WfCancelConfirmChk()
        {
            ...
            try
            {
                ...
                var listSumIfb =                         //依料號及倉庫做加總
                        ...
                        {
                            ifb03 = ifb_sum.Key.ifb03,
                            ifa06 = masterModel.ifa06,
                            ifb13 = ifb_sum.Sum(p => p.ifb13)
                        }
                    ;
                foreach (var sumIfb in listSumIfb)
                {
                    icc05 = BoInv.OfGetIcc05(sumIfb.ifb03, sumIfb.ifa06);
                    if (icc05 < sumIfb.ifb13)
                    {
                        WfShowErrorMsg(string.Format("料號{0} 庫存量{1}不足!", sumIfb.ifb03, icc05));
                        return false;
